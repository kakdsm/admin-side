...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const viewPostApplicantLimit = document.getElementById(
    "viewPostApplicantLimit"
  );

  // ---  Get textareas AND their display divs ---
  const viewPostSummary = document.getElementById("viewPostSummary");
  const viewPostResponsibilities = document.getElementById(
    "viewPostResponsibilities"
  );
  const viewPostSpecification = document.getElementById(
    "viewPostSpecification"
  );

  const displayPostSummary = document.getElementById("displayPostSummary");
  const displayPostResponsibilities = document.getElementById(
    "displayPostResponsibilities"
  );
  const displayPostSpecification = document.getElementById(
    "displayPostSpecification"
  );
  ...
  const openAddJobPostModalBtn = document.getElementById(
    "openAddJobPostModalBtn"
  );
  ...
  const closeStatusConfirmModal = document.getElementById(
    "closeStatusConfirmModal"
  );
  const cancelCloseStatusButton = document.getElementById("cancelCloseStatus");
  const confirmCloseStatusButton =
    document.getElementById("confirmCloseStatus");
  ...
  const confirmDeletePostingBtn = document.getElementById(
    "confirmDeletePosting"
  );
  const deletePostingJobRoleSpan = document.getElementById(
    "deletePostingJobRole"
  );
  ...
  const jobPostPaginationInfo = document.getElementById(
    "jobPostPaginationInfo"
  );
  ...
  closeAdminUserProfileModal?.addEventListener("click", () =>
    closeAdminProfileModal(true)
  );
  adminProfileModalCloseBtn?.addEventListener("click", () =>
  ...
  function toggleEditModeJobPost(enable) {
    ...
    fieldsToQuill.forEach((field, index) => {
      if (enable) {
        ...
        let currentInstance = [
          quillViewSummary,
          quillViewResponsibilities,
          quillViewSpecification,
        ][index];

        if (!currentInstance) {
          currentInstance = initializeQuillEditor(
            field.editorDivId,
            field.textareaId,
            field.placeholder
          );
    ...
    if (enable) {
      ...
      if (
        !viewPostDeadline.value ||
        viewPostDeadline.value === "Invalid Date"
      ) {
  ...
  function hideJobPostModal() {
    if (isJobPostEditing) {
      ...
      displayPostSummary.innerHTML =
        originalPostData.postsummary || placeholder;

      viewPostResponsibilities.value = originalPostData.postresponsibilities;
      displayPostResponsibilities.innerHTML =
        originalPostData.postresponsibilities || placeholder;

      viewPostSpecification.value = originalPostData.postspecification;
      displayPostSpecification.innerHTML =
        originalPostData.postspecification || placeholder;
  ...
  async function viewJobPost(postId) {
    try {
      ...
      if (result.success) {
        ...
        const statusClass =
          data.poststatus.toLowerCase() === "open" ? "active" : "inactive";
        ...
        displayPostResponsibilities.innerHTML =
          data.postresponsibilities || placeholder;

        viewPostSpecification.value = data.postspecification;
        displayPostSpecification.innerHTML =
          data.postspecification || placeholder;
  ...
  async function editJobPost(postId) {
    try {
      ...
      if (result.success) {
        ...
        postModalStatus.className = "profile-status";
        const statusClass =
          data.poststatus.toLowerCase() === "open" ? "active" : "inactive"; // Matched CSS classes
        postModalStatus.classList.add(statusClass);
  ...
  saveJobPostBtn?.addEventListener("click", async function () {
    const quillInstances = [
      ...
      {
        instance: quillViewResponsibilities,
        textareaId: "viewPostResponsibilities",
      },
    ...
    quillInstances.forEach((item) => {
      if (item.instance) {
        ...
        if (textarea) {
          textarea.value =
            item.instance.getLength() === 1 || html === "<p><br></p>"
              ? ""
              : html;
        }
      }
  ...
  addJobPostForm?.addEventListener("submit", function (e) {
    ...
    if (quillInstanceSummary) {
      ...
      document.getElementById("postSummary").value =
        quillInstanceSummary.getLength() === 1 || html === "<p><br></p>"
          ? ""
          : html;
    }
    if (quillInstanceResponsibilities) {
      ...
      document.getElementById("postResponsibilities").value =
        quillInstanceResponsibilities.getLength() === 1 ||
        html === "<p><br></p>"
          ? ""
          : html;
    }
    if (quillInstanceSpecification) {
      ...
      document.getElementById("postSpecification").value =
        quillInstanceSpecification.getLength() === 1 || html === "<p><br></p>"
          ? ""
          : html;
    }
  ...
  function changePostStatus(postId, action, deadline = null) {
    ...
    fetch("update_job_status.php", {
      ...
      .catch((error) => {
        console.error("Fetch error:", error);
        alert(
          "An error occurred during status update. Check console for details."
        );
      });
  }
  ...
  confirmDeletePostingBtn?.addEventListener("click", async function () {
    ...
    confirmDeletePostingBtn.innerHTML =
  ...
  function renderJobPostTable() {
    ...
    let filteredRows = allTableRows.filter((row) => {
      ...
      const matchesSearch =
        jobRole.includes(searchTerm) || postType.includes(searchTerm);
    ...
    if (paginatedRows.length === 0) {
      jobPostTableBody.innerHTML =
        '<tr><td colspan="8" style="text-align: center;">No Job Posts Found Matching Your Criteria</td></tr>';
    } else {
